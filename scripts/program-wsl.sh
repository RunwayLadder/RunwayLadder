...
export CARGO_TARGET_DIR="$HOME/.cache/runway-ladder-target"
...
case "${1:-build}" in
  build)
    ...
    cargo test --manifest-path programs/runway-ladder/Cargo.toml \
      --features idl-build --test idl_rung --quiet
    python3 scripts/idl-rung.py
    mkdir -p target/deploy packages/sdk/src/idl
    cp "$CARGO_TARGET_DIR/deploy/runway_ladder.so" target/deploy/
    ...
    cp target/idl/runway_ladder.json packages/sdk/src/idl/
    ;;
  test)
    ...
    "$0" build
    SBF_OUT_DIR="$PWD/target/deploy"       cargo test --manifest-path programs/runway-ladder/Cargo.toml
    ;;
  *)
